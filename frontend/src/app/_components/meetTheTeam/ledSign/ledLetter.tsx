...
const FlickeringLights = styled("span")<{ startTime: string, blinkNumber: number }>(
...
export default function LedLetter(props: LedLetterProps) {
  ...
  return (
    <FlickeringLights startTime={`${randomNumber}ms`} blinkNumber={3}>{props.letter}</FlickeringLights>
  );
}